...
module.exports = function (app, config) {
	...
	return function clusterStartTask(req, res) {
		...
		var taskId = uuid('t');
		var task = { started: new Date(), id: taskId, data: req.body, requestId: req.requestId };
		...
		var listenersExpireTimeString = (new Date(Number(listenersExpireTime))).toISOString();
		req.log.debug('Start task: cleanup (' + bucket + ' from 0 to ' + listenersExpireTimeString + ')');
