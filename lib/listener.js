...
var maxConcurrentForwards = env.POSTABLE_MAX_CONCURRENT_FORWARDS || 50;
...
var workQueue = queue(maxConcurrentForwards);
...
function forwardTask(task) {
	if (task) {
		...
		function tryForward(attempt) {
			...
			log.debug(
				'Attempting to forward task to (' + forwardHttpUrl + ')',
				{
				...
				workQueue.push(function (done) {
					...
					req = request({
						...
						if (e) {
							...
							forwarded = true;
							log.debug(
								'Successful task forward to local endpoint (' + forwardHttpUrl + ')',
								{
									postableForwardResponseStatus: res.statusCode,
									postableForwardResponseBody: JSON.stringify(typeof body === 'undefined' ? null : body),
									postableForwardTask: task
						...
						forwarded || tryForward(attempt + 1);
					});
