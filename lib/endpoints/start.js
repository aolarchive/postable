module.exports = function (app, config) {
	...
	return function startTask(req, res) {
		...
		var task = { time: Date.now(), id: taskId, data: req.body };
		var taskJson = JSON.stringify(task);
		var bucket = req.params.bucket;
		var timeout = Math.max(5, +req.query.timeout || 30);
		var alive = true;
		...
		function getListener(id, callback) {
			...
			app.redis.get(redisKeyListener, function (e, json) {
				var listener = null;
				if (json) {
					try {
						...
						console.error('Error parsing JSON when getting listener: ' + error);
					}
				}
		...
		app.redis.setex(redisKeyLastTask, config.listenerSetTimeoutSeconds, taskJson);
					...
					if (alive && data && data.listenerId) {
						getListener(data.listenerId, function (e, listener) {
							if (listener) {
								...
								app.redis.zrem(redisKeyPending, data.listenerId, function () {
