module.exports = function (app, config) {
	...
	return function listenForTasks(req, res) {
		...
		var intervalSeconds = Math.max(1000, Math.floor(Math.max(1, config.listenerTimeoutSeconds / 2) * 1000) - 1000);
		var intervalHandle = setInterval(function () {
			if (alive) {
				...
				buckets.forEach(function (bucket) {
					...
					var redisKeyBucketListeners = app.redis.key('listeners', bucket);
					app.redis.expire(redisKeyLastTask, config.listenerSetTimeoutSeconds);
